...
class MySnackbarManager {
    ...
    public void show(int duration, Callback callback) {
        synchronized (mLock) {
            if (isCurrentSnackbarLocked(callback)) {
                ...
                // If this is the MySnackbar currently being shown, call re-schedule it's
                // timeout
            ...
            if (mCurrentSnackbar != null && cancelSnackbarLocked(mCurrentSnackbar,
                    MySnackbar.Callback.DISMISS_EVENT_CONSECUTIVE)) {
                // If we currently have a MySnackbar, try and cancel it and wait in line
                return;
            } else {
    ...
    /**
     * Should be called when a MySnackbar is no longer displayed. This is after any exit
     * animation has finished.
     */
    public void onDismissed(Callback callback) {
        synchronized (mLock) {
            if (isCurrentSnackbarLocked(callback)) {
                // If the callback is from a MySnackbar currently show, remove it and show a new one
                mCurrentSnackbar = null;
    ...
    /**
     * Should be called when a MySnackbar is being shown. This is after any entrance animation has
     * finished.
     */
    ...
    private void showNextSnackbarLocked() {
        if (mNextSnackbar != null) {
            ...
            if (callback != null) {
                ...
                // The callback doesn't exist any more, clear out the MySnackbar
                mCurrentSnackbar = null;
            }
        }
    }
    ...
    private void scheduleTimeoutLocked(SnackbarRecord r) {
        if (r.duration == MySnackbar.LENGTH_INDEFINITE) {
        ...
        } else if (r.duration == MySnackbar.LENGTH_SHORT) {
    ...
    void handleTimeout(SnackbarRecord record) {
        synchronized (mLock) {
            if (mCurrentSnackbar == record || mNextSnackbar == record) {
                cancelSnackbarLocked(record, MySnackbar.Callback.DISMISS_EVENT_TIMEOUT);
            }
        }
    }
